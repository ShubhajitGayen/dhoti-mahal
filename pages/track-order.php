...
<div class="track-box">
    ...
    <div class="checkout-section">
        ...
        <div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:20px;">
            <div><span style="color:var(--muted);font-size:13px;">Date</span><br><strong><?= date('d M Y', strtotime($order['created_at'])) ?></strong></div>
            <div><span style="color:var(--muted);font-size:13px;">Total</span><br><strong><?= formatPrice((float)$order['total']) ?></strong></div>
            <div><span style="color:var(--muted);font-size:13px;">Payment</span><br><span class="status-badge status-<?= $order['payment_status'] ?>"><?= ucfirst($order['payment_status']) ?></span></div>
            <div><span style="color:var(--muted);font-size:13px;">Status</span><br><span class="status-badge status-<?= $order['order_status'] ?>"><?= ucfirst($order['order_status']) ?></span></div>
        </div>
            ...
            <div style="display:flex;align-items:center;min-width:400px;">
                ...
                ?>
                    <div style="display:flex;flex-direction:column;align-items:center;flex:1;position:relative;">
                        <div style="width:36px;height:36px;border-radius:50%;background:<?= $color ?>;color:white;display:flex;align-items:center;justify-content:center;font-size:14px;z-index:1;">
                            <i class="fas <?= $icon ?>"></i>
                        </div>
                        <div style="font-size:11px;margin-top:6px;color:<?= $done ? 'var(--crimson)' : 'var(--muted)' ?>;font-weight:<?= $done ? '700' : '400' ?>;text-transform:uppercase;letter-spacing:0.5px;"><?= ucfirst($st) ?></div>
                        <?php if ($i < count($statusOrder) - 1): ?>
                        <div style="position:absolute;top:18px;left:50%;width:100%;height:2px;background:<?= $done ? 'var(--crimson)' : 'var(--border)' ?>;z-index:0;"></div>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            </div>
        ...
        <!-- Items -->
        <h4 style="font-family:var(--font-head);margin-bottom:12px;border-top:1px solid var(--border);padding-top:16px;">Items in This Order</h4>
        ...
        <div style="text-align:center;margin-top:20px;">
            <p style="font-size:13px;color:var(--muted);">Need help? 
                <a href="https://wa.me/<?= sanitize(getSetting('whatsapp_number')) ?>?text=Help with order <?= urlencode($order['order_number']) ?>" style="color:#25D366;font-weight:600;">
